...
int main(int argc, char **argv)
{
    ...
    if (0 == mpi_rank) {
    fprintf(stderr, "RANK 0\n");

    PMIX_APP_CONSTRUCT(&app);
    app.cmd = strdup("./task.sh");
    ...
    PMIX_INFO_CONSTRUCT(&jinfo[2]);
    PMIX_INFO_LOAD(&jinfo[2], PMIX_OUTPUT_TO_FILE, "out", PMIX_STRING);

    fprintf(stderr, "Client %s:%u: calling PMIx_Spawn\n",
            myproc.nspace, myproc.rank);
    rc = PMIx_Spawn(jinfo, 3, &app, 1, nsp2);
